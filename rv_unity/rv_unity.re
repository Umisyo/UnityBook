...
== 開発環境
: Windows10
 一般的なOS．UnityはMac版でも操作は基本的に同じですが，今回使うWiiリモコンやペダル用いたMacでの入力方法については対象外とさせてもらうのでご了承ください．
...
=== 経路探索AIの実装
Unityには嬉しいことに経路探索AIが最初から用意されています．まず，ゾンビに「Nav Mesh Agent」のコンポーネントを追加し図のように設定します．
...
公式ドキュメントでは，NavMesh AgentはPhysicsと合わせて使用するのを非推奨としています。何故かというと、キャラの動きをNavMesh Agentで行っているのにそれに物理トリガを加えたりアニメーション動作を加えると競合してしまうからです．
